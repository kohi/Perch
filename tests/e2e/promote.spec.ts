...
test("[[関連リンク]]: 非 Tauri ではデスクトップ専用案内が出て、タグ/昇格 UI は従来どおり", async ({
  page,
}) => {
  await addTab(page, "関連リンクのメモ\n本文", 1);
  await page.getByTestId("promote-btn").click();
  await expect(page.getByTestId("promote-modal")).toBeVisible();

  // 実 Vault 候補は Tauri 必須 → 非 Tauri では控えめな案内のみ（旧・無効スロットは撤去済み）。
  await expect(page.getByTestId("promote-links-hint")).toBeVisible();
  await expect(page.getByTestId("link-suggestion")).toHaveCount(0);
  await expect(page.getByTestId("approved-link")).toHaveCount(0);

  // 回帰: タグ手動追加は動く（昇格導線を止めない）。
  await page.getByTestId("promote-tag-input").fill("手動タグ");
  await page.getByTestId("promote-tag-add").click();
  await expect(
    page.locator('[data-testid="confirmed-tag"][data-tag="手動タグ"]'),
  ).toBeVisible();
