...
セッション・スレッド側（claude.ai の伴走スレッド等）でのみ合意されたバックログ・申し送り事項は、
**その Wave のクローズ同期で STATUS（または同等の再開参照ドキュメント）に記載されるまで
「完了扱い」にしない**。

- クローズ同期の完了報告には、当該 Wave で発生したスレッド合意事項の**記載行（実出力）**を含める
- スレッド合意事項がゼロの Wave は「該当なし」と明記すればよい（暗黙の省略は不可）

理由: スレッド合意はリポジトリから見えず、次スレッド・次セッションに引き継がれない
（口頭合意のバックログが未記載のまま孤児要件化した実例あり）。記録の正本はリポジトリ側にある。
...
- **2026-07-23**: §4 に「スレッド側合意の持ち込み義務」を新設
  （Kikimiru で口頭合意バックログが孤児要件化した実例 → Wave 9 クローズで実地運用後に成文化）。
  運用中の全プロジェクト（Helm ほか）への横展開が必要
